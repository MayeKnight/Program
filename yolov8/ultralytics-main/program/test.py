...
# ---------- 2. 细胞世界管理类（允许 1/4 直径重叠）----------
class CellWorld:
    def __init__(self, image_path, model_path, conf_threshold=0.5, allowed_overlap_ratio=0.75, grid_cell_size_ratio=0.3):
        """
        allowed_overlap_ratio : 重叠允许系数，圆心距 >= (r1+r2)*ratio 视为安全。
                                ratio=0.75 表示允许重叠直径的1/4。
        """
    ...
    def plan_path_for_cell(self, cell_idx, target_px):
        """为第 cell_idx 个细胞规划到 target_px (像素坐标) 的无碰撞路径"""
        if cell_idx is None:
            return []
        ...
        for i, (ox, oy, or_) in enumerate(self.cells):
            if i == cell_idx:
                continue
            # 允许重叠半径之和的 (1 - allowed_overlap_ratio) 部分
            # 碰撞条件：圆心距 < (r + or_) * allowed_overlap_ratio
            # 因此障碍物区域是以障碍物圆心为中心、半径 = (r + or_) * allowed_overlap_ratio 的圆
    ...
    def check_collision_with_others(self, cell_idx, new_center_px):
        """检查细胞移动到新圆心时是否与其它细胞发生不允许的重叠（超过 allowed 阈值）"""
        x, y, r = self.cells[cell_idx]
        for i, (ox, oy, or_) in enumerate(self.cells):
            if i == cell_idx:
                continue
    ...
    def draw(self, img_draw):
        ...
        cv2.putText(img_draw, "Click cell to select (green) -> Drag to move (A* with 1/4 overlap allowed)", 
                    (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2.putText(img_draw, "Press 'r' reset, 'c' clear selection, ESC exit", 
...
def main():
    ...
    try:
        world = CellWorld(image_path, model_path, 
                         conf_threshold=0.5, 
                         allowed_overlap_ratio=0.75,   # 允许重叠直径的1/4
    ...
    cv2.namedWindow("Cell Separation with A* (1/4 overlap allowed)")
    ...
    cv2.setMouseCallback("Cell Separation with A* (1/4 overlap allowed)", mouse_callback)
    ...
    while True:
        ...
        world.draw(display)
        cv2.imshow("Cell Separation with A* (1/4 overlap allowed)", display)
