...
void dfs(GraphAdjList& graph, unordered_set<Vertex*>& visited, vector<Vertex*>& res, Vertex* vet)
{
    res.push_back(vet);
    visited.emplace(vet);
    for (Vertex* adjVet : graph.adjList[vet])  //这里的graph.adjList[vet]即身为graphAdjList类的对象graph中所包含adjList成员函数, adjList为哈希表, 故这里adjList[vet]即键vet对应的值, 这里的值即为与顶点'vet'相关联的所有邻接顶点的列表
    {
...
vector<Vertex* > graphDFS(GraphAdjList& graph, Vertex* startVet) //该函数类型为vector<Vertex* >则最后也应该返回这个类型
{
    //哈希集合, 用于记录已被访问过的顶点
